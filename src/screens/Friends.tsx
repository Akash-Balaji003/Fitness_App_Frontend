...
const Friends = ({ navigation }: NativeStackScreenProps<RootStackParamList, "Friends">) => {
    ...
    const fetchFriends = async () => {
        try {
            const response = await fetch(
                `https://1psc5nc9-8001.inc1.devtunnels.ms/get-friends?id=${user?.user_id}`, // http://127.0.0.1:8000 "SRM = https://9kz2rcl6-8000.inc1.devtunnels.ms Test = https://9kz2rcl6-8000.inc1.devtunnels.ms"
                {
                    method: "GET",
                    headers: {
    ...
    const fetchPendingRequests = async () => {
        try {
            const response = await fetch(`https://1psc5nc9-8001.inc1.devtunnels.ms/get-pending-requests?id=${user?.user_id}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                },
    ...
    const handleResponse = async (friendship_id: number, status: string) => {
        try {
            ...
            const response = await fetch(`https://1psc5nc9-8001.inc1.devtunnels.ms/respond-request?id=${friendship_id}&status=${status}`, {
                headers: {
                    "Content-Type": "application/json",
                },
            });
