...
const SearchUser = () => {
    ...
    useEffect(() => {
        ...
        if (searchQuery.length >= 3) {  // Start searching after 3 characters
            fetch(`https://9kz2rcl6-8000.inc1.devtunnels.ms/users/search/?name=${searchQuery}`)
    ...
    const sendFriendRequest = async (recipientId: number) => {
        try {
            const response = await fetch(
                `https://9kz2rcl6-8000.inc1.devtunnels.ms/send-request?req_id=${user?.user_id}&rec_id=${recipientId}`,
                {
